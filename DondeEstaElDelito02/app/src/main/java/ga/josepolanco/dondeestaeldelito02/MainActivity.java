...
public class MainActivity extends AppCompatActivity  implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.btn_iniciar_sesion){
            if(txt_usuario.getText().toString().trim().length()==0 ||
                    txt_contraseña.getText().toString().trim().length()==0){
                Toast.makeText(getApplicationContext(), "Inició Sesión", Toast.LENGTH_SHORT).show();
        ...
        if(v.getId() == R.id.txt_login_registrar){
